...
class MultipleImageDDPSampler(DDPSampler):
    def __init__(self, batch_size, mask, num_replicas, rank, total_len, epoch_size, tpu):
        super(MultipleImageDDPSampler, self).__init__(
            batch_size, num_replicas, rank, tpu
        )
        self.total_len = total_len
        total_len_epoch = self.total_len

        if mask is not None:
            self.mask_images = mask.squeeze() >= 0.5
            total_len_epoch = int(self.mask_images.sum()) # total len decreases if there's a mask
        else:
            self.mask_images = None
        self.epoch_size = int(log(1-0.8)//log((total_len_epoch - self.batch_size)/total_len_epoch))
    ...
    def __iter__(self):
        ...
        if self.mask_images is not None:
            indices = np.stack(
                [
                    np.random.choice(full_index[self.mask_images], self.batch_size)
