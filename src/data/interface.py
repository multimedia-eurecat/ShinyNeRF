...
class LitData(pl.LightningDataModule):
    def __init__(
        ...
        if not hasattr(self, "distances"):
            self.distances = None
        if not hasattr(self, "mask_images"):
            self.mask_images = None
        if not hasattr(self, "bitangents"):
    ...
    def setup(self, stage):
        ...
        if stage == "fit" or self.needs_train_info:
            self.train_dset, _, self.train_mask = self.split_each(
                self.images, self.normals, self.bitangents, None, self.i_train, self.distances, self.mask_images, dummy=False
            )
            self.train_image_sizes = self.image_sizes[self.i_train]

        if stage == "fit" or stage is None:
            self.val_dset, self.val_dummy, self.val_mask = self.split_each(
                self.images, self.normals, self.bitangents, None, self.i_val, self.distances, self.mask_images, dummy=True
            )
            self.val_image_sizes = self.image_sizes[self.i_val]

        if stage == "test" or stage is None:
            test_idx = self.i_test if self.eval_test_only else self.i_all
            self.test_dset, self.test_dummy, _ = self.split_each(
                self.images, self.normals, self.bitangents, None, test_idx, self.distances, None, dummy=True
            )
        ...
        if stage == "predict" or stage is None:
            ...
            self.predict_dset, self.pred_dummy, _ = self.split_each(
                None, None, render_poses, np.arange(len(render_poses), None)
            )
    ...
    def split_each(
        ...
        if _bitangents is not None:
            bitangents_idx = np.concatenate([_bitangents[i].reshape(-1, 4) for i in idx])
            bitangents = np.zeros((n_dset + dummy_num, 4))
            bitangents[:n_dset] = bitangents_idx
            bitangents[n_dset:] = bitangents[:dummy_num]


        if _distances is not None:
            distance_idx = np.concatenate([_distances[i].reshape(-1, 4) for i in idx])
            distances = np.zeros((n_dset + dummy_num, 4))
            distances[:n_dset] = distance_idx
            distances[n_dset:] = distances[:dummy_num]

        if _mask_images is not None and _mask_images.shape[0] != 0:
            mask_images_idx = np.concatenate([_mask_images[i].reshape(-1, 1) for i in idx])
            mask_images = np.zeros((n_dset + dummy_num, 1))
            mask_images[:n_dset] = mask_images_idx
            mask_images[n_dset:] = mask_images[:dummy_num]
        else:
            mask_images = None
        ...
        rays_info = {
            ...
            "distances": distances,
            "image_index": idx.repeat(rays_p_image),
        }

        return RaySet(rays_info), dummy_num, mask_images
...
class RaySet(Dataset):
    def __init__(self, rays_info):
        ...
        self.distances = rays_info["distances"]

        # Anisotropic model
        self.bitangents = rays_info["bitangents"]

        self.N = len(self.rays_d)

        self.image_indx =  rays_info["image_index"]
    ...
    def __getitem__(self, index):
        ...
        ret["bitangents"] = np.zeros_like(ret["rays_o"])
        ret["distances"] = np.zeros_like(ret["rays_o"])
        ret["image_indx"] = self.image_indx[index]
        ...
        if self.bitangents is not None:
            ret["bitangents"] = torch.from_numpy(self.bitangents[index])

        if self.distances is not None:
            ret["distances"] = torch.from_numpy(self.distances[index])
