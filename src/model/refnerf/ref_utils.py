...
def compute_weighted_mae_picture(weights, normals, normals_gt):
    """Compute weighted mean angular error, assuming normals are unit length."""
    one_eps = 1 - torch.finfo(torch.float32).eps
    return (
        (
        ...
        / (torch.sum(weights, -1) + torch.finfo(torch.float32).eps)
        * 180.0
        / np.pi
    )
...
def compute_weighted_mae_chunked(weights, normals, normals_gt, chunk_size):
    """Compute weighted mean angular error in chunks to save VRAM."""
    one_eps = 1 - torch.finfo(torch.float32).eps
    total_weighted_error = 0.0
    total_weight = 0.0

    for i in range(0, weights.size(0), chunk_size):
        weights_chunk = weights[i:i+chunk_size]
        normals_chunk = normals[i:i+chunk_size]
        normals_gt_chunk = normals_gt[i:i+chunk_size]

        weighted_error = (
            weights_chunk
            * torch.arccos(
                torch.clamp(torch.sum(normals_chunk * normals_gt_chunk, -1), -one_eps, one_eps)
            )
        ).sum()

        total_weighted_error += weighted_error
        total_weight += weights_chunk.sum()

    return total_weighted_error / total_weight * 180.0 / np.pi
