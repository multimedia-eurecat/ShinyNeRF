...
class LitModel(pl.LightningModule):
    ...
    def alter_gather_cat(self, outputs, key, image_sizes):
        ...
        each = torch.cat([output[key] for output in outputs]).cpu() # move this to ram (cpu)
        ...
        for (h, w) in image_sizes:
            ret.append(all[curr : curr + h * w].reshape(h, w, all.size()[-1]))
    ...
    def on_predict_epoch_end(self, outputs):
        ...
        image_dir = os.path.join(self.logdir, "render")
        os.makedirs(image_dir, exist_ok=True)


        if "distance" in outputs[0][0]:
            
            distance = self.alter_gather_cat(outputs[0], "distance", all_image_sizes)

            if self.trainer.is_global_zero:
                distance_dir = os.path.join(image_dir, "distance")
                #normal_pred_dir = os.path.join(image_dir, "normals_pred")
                
                os.makedirs(distance_dir, exist_ok=True)
                #os.makedirs(normal_pred_dir, exist_ok=True)
                dist_3d = [dist.repeat(1,1,3) for dist in distance]
                store_image.store_image(distance_dir, dist_3d)
                #store_image.store_image(normal_pred_dir, normals_pred)
            del distance


        if "comp_normals" in outputs[0][0]:
            normals = self.alter_gather_cat(outputs[0], "comp_normals", all_image_sizes)

            if self.trainer.is_global_zero:
                normal_dir = os.path.join(image_dir, "comp_normals")
                #normal_pred_dir = os.path.join(image_dir, "normals_pred")
                
                os.makedirs(normal_dir, exist_ok=True)
                #os.makedirs(normal_pred_dir, exist_ok=True)
                
                store_image.store_image(normal_dir, normals)
                #store_image.store_image(normal_pred_dir, normals_pred)
            del normals

        if "comp_normals_pred" in outputs[0][0]:
            normals_pred = self.alter_gather_cat(outputs[0], "comp_normals_pred", all_image_sizes)

            if self.trainer.is_global_zero:
                normal_pred_dir = os.path.join(image_dir, "comp_normals_pred")
        ...
        if "comp_tangent_pred" in outputs[0][0]:
            normals = self.alter_gather_cat(outputs[0], "comp_normals", all_image_sizes)
            tangent_pred = self.alter_gather_cat(outputs[0], "comp_tangent_pred", all_image_sizes)
            if self.trainer.is_global_zero:
                tangent_dir = os.path.join(image_dir, "comp_tangent_pred")
                os.makedirs(tangent_dir, exist_ok=True)            
                store_image.store_image(tangent_dir, tangent_pred)


                tanxnorm_dir = os.path.join(image_dir, "comp_tangent_x_norm")
                os.makedirs(tanxnorm_dir, exist_ok=True)            
                store_image.store_image(tanxnorm_dir,  [abs((t * normal ).sum(axis=-1)) for t, normal in zip(tangent_pred, normals)])
        ...
        if self.trainer.is_global_zero:
            rgbs_folder = os.path.join(image_dir, "rgb")
            os.makedirs(rgbs_folder, exist_ok=True)
            store_image.store_image(rgbs_folder, rgbs)
            store_image.store_video(rgbs_folder, rgbs, None)

        return None
    def average_difference(self, normals, normals_pred):
        n = len(normals)
        total_difference = 0.0

        for i in range(n):
            diff = np.linalg.norm(normals[i].cpu() - normals_pred[i].cpu())
            total_difference += diff

        average_diff = total_difference / n
        return average_diff
