...
class MipNeRF360(nn.Module):
    ...
    def forward(self, batch, train_frac, randomized, is_train, near, far, radius):

        bsz, _ = batch["rays_o"].shape
        device = batch["rays_o"].device

        if near is None:
            distance_to_center = torch.sqrt(batch["rays_o"].T[0]**2 + batch["rays_o"].T[1]**2 + batch["rays_o"].T[2]**2)
...
class LitMipNeRF360(LitModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        rendered_results, ray_history = self.model(
            batch, train_frac, True, True, self.near, self.far, self.radius
        )
    ...
    def render_rays(self, batch, batch_idx):
        ...
        train_frac = self.global_step / max_steps
        rendered_results, ray_history = self.model(
            batch, train_frac, False, False, self.near, self.far, self.radius
        )
        rgb = rendered_results[-1]["rgb"]
        target = batch["target"]
        ret["target"] = target
