...
class LitDVGO(LitModel):
    ...
    def compute_bbox_by_cam_frustrm_bounded(self):
        ...
        if self.near is None:
            distance_to_center = torch.sqrt(rays_o.T[0]**2 + rays_o.T[1]**2 + rays_o.T[2]**2)
            self.near = distance_to_center - radius + 1
            self.far = distance_to_center + 3

        ''' # this should go in sample_along_rays inside helper.py
